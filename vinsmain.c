...
#define RX_FIFO_ADDRESS 0x400
#define RX_FIFO_CONTROL_ADDRESS 0x480
#define BUFFER_SIZE 64

// MCP2518FD SPI instructions
#define MCP2518FD_RESET 0x00
...
int initialize_mcp2518fd(int spi_handle) {
    uint8_t tx_buffer[4] = {0x02, (REG_CiCON >> 8) & 0xFF, REG_CiCON & 0xFF, 0x40}; // Set Configuration mode
...
int switch_to_normal_mode(int spi_handle) {
    uint8_t tx_buffer[4] = {0x02, (REG_CiCON >> 8) & 0xFF, REG_CiCON & 0xFF, 0x00}; // Set Normal mode
    uint8_t rx_buffer[4] = {0};
...
void configure_baud_rate(int spi_handle) {
    uint8_t tx_buffer[5] = {0x02, (REG_CiNBTCFG >> 8) & 0xFF, REG_CiNBTCFG & 0xFF, 0x87, 0x01}; // Set 1 Mbps baud rate
...
void setup_rx_fifo(int spi_handle) {
    uint8_t tx_buffer[8] = {0x02, (RX_FIFO_CONTROL_ADDRESS >> 8) & 0xFF, RX_FIFO_CONTROL_ADDRESS & 0xFF, 0x00, 0x00, 0x03, 0x00, 0x00}; // RX FIFO setup
...
void setup_catch_all_filter(int spi_handle) {
    uint8_t tx_buffer[8] = {0x02, (REG_CiFIFOCON >> 8) & 0xFF, REG_CiFIFOCON & 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00}; // Filter setup
    uint8_t rx_buffer[8] = {0};
...
void read_can_message(int spi_handle) {
    uint8_t tx_buffer[16] = {0x03, (RX_FIFO_ADDRESS >> 8) & 0xFF, RX_FIFO_ADDRESS & 0xFF};
